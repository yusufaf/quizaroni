...
import { SORT_DIRECTIONS } from "utilities/constants";
...
export default function useSortStudysets(props: UseSortStudysetsParams): Studyset[] {
    ...
    const sortedStudysets = useMemo(() => {
        ...
        if (selectedSort) {
            localSortedStudysets.sort((a: Studyset, b: Studyset) => {
                const sortValueA = selectedSort === CARDS_LENGTH_PROPERTY ? a.cards.length : a[selectedSort];
                const sortValueB = selectedSort === CARDS_LENGTH_PROPERTY ? b.cards.length : b[selectedSort];
