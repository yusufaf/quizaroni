...
const HomeToolbar = ({
  ...
  const handleMassActionClick = (type: string) => {
    const modifiedType = multipleStudysetsSelected ? `${type}_MULTIPLE` : type;
  ...
  const sortControls = (
    <SimpleFlexContainer>
      ...
      <FormControl sx={{ minWidth: "8.75rem" }} size="small">
        <InputLabel id="sort-label">{t("home.toolbar.sortBy")}</InputLabel>
        <Select
          labelId="sort-label"
        ...
        >
          ...
          <MenuItem value={"title"}>{t("home.columns.title")}</MenuItem>
          <MenuItem value={"lastViewed"}>
            {t("home.columns.lastViewed")}
          </MenuItem>
          <MenuItem value={"createdAt"}>{t("home.columns.created")}</MenuItem>
  ...
  return (
    <SpacedFlexContainer style={{ alignItems: "baseline" }}>
      {isTableView ? (
        ...
        <>
          ...
          <Dialog
            ...
            fullWidth
          >
            <DialogTitle>{t("home.toolbar.filtersDialogTitle")}</DialogTitle>
            <DialogContent
              sx={{
                display: "flex",
      ...
      {isTableView && selectedStudysetRows.length > 0 && (
        <SimpleFlexContainer style={{ gap: "1rem" }}>
          {!(selectedStudysetRows.length > 1) && (
            <>
              <Button startIcon={<EditRounded />} onClick={handleEditClick}>
                Edit
              </Button>
              <Button
                startIcon={
              ...
              >
                {firstStudyset?.favorited ? "Unfavorite" : "Favorite"}
              </Button>
            </>
          )}
          <Button
            ...
            onClick={() =>
              handleMassActionClick(STUDYSET_CONFIRM_DIALOGS.DUPLICATE)
            }
          >
            Duplicate
          </Button>
          <Button
            ...
            onClick={() =>
              handleMassActionClick(STUDYSET_CONFIRM_DIALOGS.DELETE)
            }
          >
            Delete
          </Button>
      ...
      <ToggleButtonGroup
        ...
        sx={{ marginLeft: "auto" }}
      >
        <ToggleButton value={HOME_LAYOUTS.TABLE} key="left" title="Table View">
          <TableViewIcon />
        </ToggleButton>
        <ToggleButton value={HOME_LAYOUTS.GRID} key="center" title="Grid View">
          <GridViewIcon />
        </ToggleButton>
        <ToggleButton
