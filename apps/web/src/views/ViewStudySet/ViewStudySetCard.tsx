...
const ViewStudySetCard = ({ card, index, selectedStudyset }: Props) => {
  ...
  const selectedLabelTerminology = labelTerminology ?? LABEL_TERMINOLOGIES.CARD;
  ...
  const getSelectedVoice = (): SpeechSynthesisVoice | null => {
    ...
    return (
      window.speechSynthesis.getVoices().find((v) => v.voiceURI === ttsVoice) ||
      null
    );
  };
    ...
    >
      {!contentOnly && (
        <SimpleFlexContainer>
          ...
          <CategoryChips
            hasIndex={selectedStudyset?.metadata?.cardIndexVisible !== false}
          >
            {categories.length > 3 ? (
